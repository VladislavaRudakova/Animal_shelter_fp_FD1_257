...
и обеспечением передержки до нахождения хозяев, а также информационно
&mdash; поделиться профилями наших питомцев в социальных сетях`
...
const textContentSchool = `У нас работает кинологическая школа и занятия с
зоопсихологом для воспитания вашего питомца, а также проходят курсы фелинологии,
благодаря которым вы сможете лучше понимать свою кошку`
  ...
  if (targetItem.id === buttonId) {
    switch (buttonId) {
      case 'take-pet':
        ...
        hiddenTitle.textContent='забрать питомца';
        hiddenRef.textContent='заполнить анкету';
        console.log(hiddenText + 'HIDDEN');       
        break;
        case 'help':
        hiddenText.innerHTML=textContentHelp;
        hiddenTitle.textContent='помочь проекту'
        hiddenRef.textContent='помочь';
        break;
        case 'clinic':
        hiddenText.textContent=textContentClinic;
        hiddenTitle.textContent='ветеринарная клиника'
        hiddenRef.textContent='записаться';
        break;
        case 'events':
        hiddenText.textContent=textContentEvents;
        hiddenTitle.textContent='события'
        hiddenRef.textContent='подробнее';
        break;
        case 'school':
        hiddenText.textContent=textContentSchool;
        hiddenTitle.textContent='обучение'
        hiddenRef.textContent='записаться';
        break;
    }
...
console.log('ID'+textContentTakePet);
  switch (id) {
      ...
      case 'take-pet':
        ...
        reference.innerHTML=`<a href="animal_shelter_form_take_pet.html"><div>заполнить анкету</div>
        <div class="arrow-info-block-ref"><custom-icon name="arrow-right"></custom-icon></div></a>`;
        console.log(textContainer.textContent);       
        break;
        case 'help':
        textContainer.innerHTML=textContentHelp;
        title.textContent ='помочь проекту'
        reference.innerHTML=`<a><div>помочь</div>
        <div class="arrow-info-block-ref"><custom-icon name="arrow-right"></custom-icon></div></a>`;
        break;
        case 'clinic':
        textContainer.textContent=textContentClinic;
        title.textContent='ветеринарная клиника'
        reference.innerHTML=`<a><div>записаться</div>
        <div class="arrow-info-block-ref"><custom-icon name="arrow-right"></custom-icon></div></a>`;
        break;
        case 'events':
        textContainer.textContent=textContentEvents;
        title.textContent='события'
        reference.innerHTML=`<a><div>подробнее</div>
        <div class="arrow-info-block-ref"><custom-icon name="arrow-right"></custom-icon></div></a>`;
        break;
        case 'school':
        textContainer.textContent=textContentSchool;
